...
export const editRole = async (data: TParamsEditRole) => {
  ...
  try {
    const res = await instanceAxios.put(`${API_ENDPOINT.SYSTEM.ROLE.INDEX}/${data.idRole}`, rests)
...
export const deleteRole = async (idRole: string) => {
  try {
    const res = await instanceAxios.delete(`${API_ENDPOINT.SYSTEM.ROLE.INDEX}/${idRole}`)
...
export const getDetailRole = async (idRole: string) => {
  try {
    const res = await instanceAxios.get(`${API_ENDPOINT.SYSTEM.ROLE.INDEX}/${idRole}`)
